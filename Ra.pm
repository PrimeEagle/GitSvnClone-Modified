	...
	while (1) {
		...
		foreach my $r (sort {$a <=> $b} keys %revs) {
			...
			foreach my $gs ($self->match_globs(\%exists, $paths,
				...
				#print STDERR "Debug: log_entry = ", Dumper($log_entry), "\n";
				if ($log_entry) {
					#print STDERR "Before renaming: ", Dumper($log_entry), "\n";

					#if ($log_entry->{name} =~ /^\s+$/) { # Check if the name is just spaces
					#	$log_entry->{name} = "renamed_file_" . $n_counter;
					#	$n_counter++; # Increment the counter for next use
					#	print STDERR "Renaming performed. New name: ", $log_entry->{name}, "\n";
					#} else {
					#	print STDERR "Renaming not needed for this entry.\n";
					#}
					# Right before the $gs->do_git_commit($log_entry) line
					if ($log_entry->{name} =~ /^\s+$/) { # Check if the name is just spaces
						$log_entry->{name} = "renamed_name_" . $n_counter;
						$n_counter++;
					}
					if ($log_entry->{commit_name} =~ /^\s+$/) {
						$log_entry->{commit_name} = "renamed_commit_name_" . $cn_counter;
						$cn_counter++;
					}
					#print STDERR "About to commit: ", Dumper($log_entry), "\n";
